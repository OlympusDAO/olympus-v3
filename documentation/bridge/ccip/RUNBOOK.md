...
- The OHM fee entry of step 0 of the non-Ethereum rollout lives on the source chain's `FeeQuoter 2.0.0` for a 1.6 or 2.0 lane (one quoter per chain, shared by both on-ramps) and on the lane's dedicated `EVM2EVMOnRamp` for a 1.5 lane. The scripts dispatch on the on-ramp's `typeAndVersion` and fail closed on any other generation, so a migration to a fourth one stops the readiness report and the proposal build until the reader is extended.
...
5. **Readiness gate and submission.** `./shell/ccip/check_rollout_readiness.sh` must be green for mainnet and all four chains (deployment bindings, ownership, registry, roles, backing, and every lane budget); then submit proposal id 19.
