...
5. **Readiness gate and submission.** `./shell/ccip/check_rollout_readiness.sh` must be green for mainnet and all four chains (deployment bindings, ownership, registry, roles, backing, and every EVM lane budget); then submit proposal id 19.
...
8. **Finalize (per chain, DAO MS, immediately after execution).**
    ...
    `pool.enable` and `TokenAdminRegistry.setPool(OHM, pool)`, in that order, which opens the chain in both directions toward mainnet and the other burn/mint chains. The order between the chains does not matter.

9. **Periphery (mainnet and per chain, DAO MS).** `CCIPBridgeBatch.reconcileTrustedRemotes` converges the trusted remotes and gas limits to the `periphery` blocks: on a burn/mint chain the EVM remotes with their 275000 gas limit. `CCIPBridgeBatch.enable` then switches the peripheries of the burn/mint chains on.
10. **Stranded-message tails.** Between the proposal execution and a chain's finalize, a send toward that chain (from mainnet or from an already finalized chain) burns or locks at the source and parks in `FAILURE` on the destination (`NotACompatiblePool`). That window is minutes long when the batches run back to back, and the messages are recovered permissionlessly once the destination is finalized: `OffRamp.manuallyExecute` with the execution report on a 1.5 or 1.6 lane (and a raised token gas override if the delivery ran out of budget), or a re-submission of `OffRamp.execute` with the same encoded message and CCV attestations on a 2.0.0 lane, where a `FAILURE` message can be re-executed at once and the pool call is bounded only by the transaction gas; the CCIP Explorer's manual execution covers both. The raised fee budget from step 0 already covers the delivery. The same recovery applies to any message that trips a contained inbound bucket later.
...
### Adding the Solana routes of the burn/mint chains

The pair between a burn/mint chain and Solana is opened after that chain has finalized, one chain at a time: the EVM side through the shared tooling, the Solana side by the owner of the Solana pool. The Solana pool (`4N7bZnVSC1GE27vo2Bv6kG983brwhT68BGsLuzH5nTQp`) is owned by `GoFoFfEDgALWRTw5dSY3VZQSwbFpvBEhDv1sFAWzYpbf`, the signer PDA of Chainlink's CCIP timelock on Solana (program `DoajfR5tK24xVw51fWcawUZWhAXD8yrBJVacc13neVQA`, minimum delay 3 hours), and no Olympus key holds a role on the pool or in that timelock (read 2026-09-10), so every change on the Solana side is a request to Chainlink until the pool ownership is transferred to Olympus. The Solana pool's per-chain configs are not read or written from Foundry; check them by hand against the same `env.json` entries.

1. **Chainlink fee budget (outside the repository).** The lane from Solana toward the chain needs an enabled OHM fee entry with `destGasOverhead >= 175000` on the Solana `FeeQuoter` program (the delivery mints through MINTR); none of the four chains has one (read 2026-09-10). The lane toward Solana needs no request: it runs under the fee quoter's default budget for the SVM destination, as the mainnet to Solana route does. The EVM scripts cannot read the Solana fee quoter, so confirm the entry with Chainlink before the route opens.
2. **Solana side (request to Chainlink, executed by the pool owner).** One per-chain config on the Solana pool: the remote token (the chain's OHM, stored as the 32-byte left-padded address), the accepted remote pool (its `CCIPBurnMintTokenPool`, stored as the raw 20-byte address, the form in which the Solana off-ramp hands the pool the source pool address; both as the pool's mainnet config holds them, read 2026-09-10) and both limits at the values the chain declares in `routes.solana` (inbound on Solana is the chain to Solana direction).
3. **EVM side (DAO MS as `bridge_admin`).** Declare `routes.solana` on the chain with both limits and the token-only `periphery` block (`gasLimit: 0`, `svmReceiver: bytes32(0)`), add `solana` to `olympus.config.CCIPCrossChainBridge.chains`, run `CCIPRouteReconcileBatch.reconcileRoutes` (it queues `addChain` on the local config timelock; the fee budget gate does not cover a lane toward an SVM destination and prints a note for it, as on mainnet), execute the action after the delay with `executeReadyActions`, then `CCIPBridgeBatch.reconcileTrustedRemotes`.
4. **Order.** Queue the EVM action first (its delay is one day), have the Solana config applied, and execute the queued action right after it. The chain to Solana direction stays closed until the EVM route exists (the source pool rejects the selector, so nothing burns); a send from Solana in the gap parks in `FAILURE` on the chain until the route is added, recovered by manual execution as in the stranded-message tails above. Keep the gap short, and confirm afterwards that `reconcileRoutes` and `reconcileTrustedRemotes` propose nothing.
...
- Contain one route (writes `{isEnabled: true, capacity: 2, rate: 1}` to both buckets, so every real transfer fails immediately; not gated on the config policy being enabled). The `emergency`, `admin`, `bridge_admin` and `bridge_rate_limiter` roles can all call it, so either multisig acts on its own. Emergency MS:
    ...
    DAO MS as `bridge_admin`: the same batch without the `EmergencyMS` suffix (`--function disableChain`), which reads the args entry of that name. The `remoteChain` argument names the route to contain, so the args file is chain specific: the unsuffixed file names the Solana route (live on mainnet; on a burn/mint chain once its Solana route has been added), and `CCIPTokenPoolConfigBatch_disableChain_mainnet.json` names the mainnet route, which only a burn/mint chain has.
