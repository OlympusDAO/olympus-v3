...
| OHM | Uniswap V3 OHM/WETH 30 bps | `uniswapOhmWethThirtyBps` | N/A | 1,500 sec (25 min) | N/A |
| OHM | Uniswap V3 OHM/WETH 100 bps | `uniswapOhmWethOneHundredBps` | N/A | 1,500 sec (25 min) | N/A |
...
- The price of OHM will be determined by four separate feeds across three paths: OHM/sUSDS (via USDS through `ERC4626.getPriceFromUnderlying(sUSDS)`), two wETH pools using different Uniswap V3 fee tiers, and the Chainlink OHM-ETH × ETH-USD derived feed.
    - After any zero value or deviating values (> 2% from the median) have been excluded, the average is taken.
    - Strict mode remains enabled, so OHM requires two valid prices and can tolerate multiple failed OHM feeds as long as at least two non-deviating prices remain.
...
| ----- | ---- | ------------------ |
| OHM | Uniswap V3 OHM/WETH 30 bps | 1,500 sec (25 min) |
| OHM | Uniswap V3 OHM/WETH 100 bps | 1,500 sec (25 min) |
...
sequenceDiagram
    participant User
    participant OHM
    participant OHM_WETH_03_Pool as OHM/wETH 30 bps Pool
    participant OHM_WETH_1_Pool as OHM/wETH 100 bps Pool
    ...
    par OHM/wETH 30 bps Path
        OHM->>OHM_WETH_03_Pool: getPrice(OHM/wETH 30 bps)
        OHM_WETH_03_Pool->>WETH: getPrice(wETH)
        ...
        Note over WETH: Filter: Exclude zero and values deviating >5% from median<br/>Average: Sum of valid values / count
        WETH-->>OHM_WETH_03_Pool: wETH price
        OHM_WETH_03_Pool-->>OHM: OHM/wETH 30 bps price
    and OHM/wETH 100 bps Path
        OHM->>OHM_WETH_1_Pool: getPrice(OHM/wETH 100 bps)
        OHM_WETH_1_Pool->>WETH: getPrice(wETH)
        ...
        Note over WETH: Filter: Exclude zero and values deviating >5% from median<br/>Average: Sum of valid values / count
        WETH-->>OHM_WETH_1_Pool: wETH price
        OHM_WETH_1_Pool-->>OHM: OHM/wETH 100 bps price
