...
- The `LZEndpointDelegate` inbound-channel management primitives (`skip`, `nilify`, `burn`, `clear`) are the exception: they are gated directly to `bridge_channel_manager` / `bridge_admin` / `admin`. The `bridge_channel_manager` role is an optional dedicated role for these primitives and is not granted to anyone by default.
