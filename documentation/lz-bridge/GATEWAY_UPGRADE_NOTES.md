...
Operators do not call these mutators directly. The flow is: call the `queue*` helpers on the config policy (or `queueBatch` for atomic multi-action updates), then call `executeQueuedAction` after the policy's timelock has elapsed. The emergency role can cancel a queued action at any time before execution.

The one-shot exceptions are `LZBridgeGateway.initializeBridgedSupply` (kept under `bridge_admin` / `admin` so the DAO MS can write the initial bridged supply immediately after OCG) and the bootstrap call to `LZCrossChainBridge.setConfigurator` (kept under the owner so the configurator variable can be seeded post-deploy). After both bootstrap calls and the corresponding role / configurator grants, the policy's timelock queue is the only path.
