...
- The `LZEndpointDelegate` inbound-channel management primitives (`skip`, `nilify`, `burn`, `clear`) are the exception: they are gated directly to `bridge_admin` / `admin` and are intentionally NOT routed through the timelock, so an operator (the DAO MS, which retains `bridge_admin`) can react promptly to an undeliverable or malicious inbound message.
