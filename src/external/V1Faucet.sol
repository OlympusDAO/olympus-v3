...
contract Faucet_V1 is OlympusAccessControlled {
    /*================== ERRORS ==================*/
    ...
    constructor(
        ...
        DAI = IERC20(dai_);
        mintable.push(IFaucet(gdao));
        stakingV2 = IStaking(stakingV2_);

        mintable[0].approve(stakingV2_, type(uint256).max);
    }
    ...
    function mintSGDAO(uint256 gdaoIndex_) external {
        ...
        if (gdaoIndex_ > 1) {
            ...
            // stakingV2.stake(msg.sender, 10000000000, true, false);
            // stakingV2.claim(msg.sender, true);
        }
    }

    function mintXGDAO() external {
        if (!_beenADay(lastMint[msg.sender], block.timestamp)) revert CanOnlyMintOnceADay();
