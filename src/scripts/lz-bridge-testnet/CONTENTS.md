...
- `LZTestnetConfig.sol` — testnet LayerZero addresses (endpoint, Uln302 libs, executor, DVNs),
  V2 EIDs, the 3-DVN route helper, confirmations, rate limits, and the 3-chain mesh helpers.
- `LZTestnetMockStack.sol` — library that deploys a minimal real Kernel stack
  (Kernel + ROLES + RolesAdmin + MINTR + OHM) on a chain that has none (arbitrum-sepolia).
- `LZBridgeTestnetBase.sol` — shared base: resolve the active chain from `block.chainid`,
