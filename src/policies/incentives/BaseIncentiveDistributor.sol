...
abstract contract BaseIncentiveDistributor is
    Policy,
    PolicyEnabler,
    IIncentiveDistributor,
    IVersioned
{
    // ========== CONSTANTS & IMMUTABLES ========== //

    /// @notice Role that can update merkle roots
    bytes32 public constant ROLE_INCENTIVE_MANAGER = "incentive_manager";
    ...
    constructor(address kernel_, uint40 lastEpochEndDate_) Policy(Kernel(kernel_)) {
        if (kernel_ == address(0)) revert IncentiveDistributor_InvalidAddress();
    ...
    function _setMerkleRoot(uint40 epochEndDate_, bytes32 merkleRoot_) internal virtual {
        // Validate input
        if (merkleRoot_ == bytes32(0)) revert IncentiveDistributor_InvalidMerkleRoot();

        // Ensure the epoch hasn't already been set
        if (epochMerkleRoots[epochEndDate_] != bytes32(0))
            revert IncentiveDistributor_EpochAlreadySet(epochEndDate_);
        ...
        if (epochEndDate_ < lastEpochEndDate + MIN_EPOCH_DURATION) {
            revert IncentiveDistributor_EpochTooEarly();
        }
    ...
    function _validateEpochEndOfDay(uint256 epochEndDate_) internal pure {
        if ((epochEndDate_ + 1) % 1 days != 0) revert IncentiveDistributor_InvalidEpochTimestamp();
    }
    ...
    function _validateClaimArrays(
        ...
        if (epochEndDates_.length == 0) revert IncentiveDistributor_NoEpochsSpecified();
        if (epochEndDates_.length != amounts_.length || epochEndDates_.length != proofs_.length) {
            revert IncentiveDistributor_ArrayLengthMismatch();
        }
    }
    ...
    function _verifyProof(
        ...
        if (hasClaimed[user_][epochEndDate_]) {
            if (revertOnFailure_) revert IncentiveDistributor_AlreadyClaimed(epochEndDate_);
            return false;
        }
    ...
    function _validateAndMarkClaimed(
        ...
        if (epochMerkleRoots[epochEndDate_] == bytes32(0))
            revert IncentiveDistributor_MerkleRootNotSet(epochEndDate_);
