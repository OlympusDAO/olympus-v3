...
contract OperatorTest is Test {
    ...
    function setUp() public {
...

        {
            ...
            PRICE.setPrice(address(wrappedReserve), 1e18 + 5e16);
            PRICE.setMovingAverage(address(ohm), 100e18);
            PRICE.setMovingAverage(address(reserve), 1e18);
            PRICE.setMovingAverage(address(reserve), 1e18 + 5e16);
        }
        {
    ...
    //         assertTrue(RANGE.active(true));
    //         assertTrue(RANGE.active(false));
    //         assertEq(TRSRY.withdrawApproval(address(operator), reserve), RANGE.capacity(false));
