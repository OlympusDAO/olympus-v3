...
contract BurnerLoansConfigTimelockExecuteTest is BurnerLoansConfigTimelockConfigGuardsTest {
    ...
    function test_givenYieldRecipientActionMatured_permissionlessExecutionAppliesRecipient(
        ...
        assertEq(burnerLoans.getYieldRepurchaseRecipient(), recipient, "yield recipient");
    }

    function test_givenYieldAssetRoutingActionMatured_executes() public {
        address recipient = address(_deployUsdsYieldRecipient());
        vm.prank(admin);
        ...
        _assertRepurchaseRouting(address(usds), 5_000);
    }

    function test_givenYieldBpsForDifferentAssetsMatured_executesBoth() public {
        MockYieldRepurchaseRecipient recipient = _deployUsdsYieldRecipient();
        ...
        actions[1] = _yieldAction(
            IBurnerLoansConfig.setYieldAssetRouting.selector,
            abi.encode(address(secondAsset), _repurchaseRouting(5_000))
        );
        vm.prank(burnerLoansAdmin);
        uint64 actionId = configTimelock.queueBatch(actions);
        vm.warp(block.timestamp + configTimelock.timelockDelay());
        ...
        _assertRepurchaseRouting(address(usds), 2_500);
        _assertRepurchaseRouting(address(secondAsset), 5_000);
    }
    ...
    function test_givenLaterYieldSubActionFails_reverts() public {
        ...
        vm.prank(burnerLoansAdmin);
        uint64 actionId = configTimelock.queueBatch(actions);
        recipient.setVaultConfig(address(usdsVault), address(usds), false);
        vm.warp(block.timestamp + configTimelock.timelockDelay());

        vm.expectPartialRevert(
            IBurnerLoans.BurnerLoans_YieldRepurchaseRecipientAssetNotEnabled.selector
        );
        configTimelock.executeQueuedAction(actionId);
        ...
        _assertRepurchaseRouting(address(usds), 0);
    }
    ...
    function test_givenYieldRecipientChangesAfterQueue_reverts() public {
        ...
        vm.prank(admin);
        burnerLoansConfig.setYieldRepurchaseRecipient(firstRecipient);
        ITimelockBatchQueue.BatchAction[] memory actions = new ITimelockBatchQueue.BatchAction[](1);
        actions[0] = _yieldAction(
            IBurnerLoansConfig.setYieldAssetRouting.selector,
            abi.encode(address(usds), _repurchaseRouting(5_000))
        ...
        vm.prank(admin);
        burnerLoansConfig.setYieldRepurchaseRecipient(secondRecipient);
        bytes32 currentHash = _yieldAssetRoutingStateHash(address(usds));
        vm.warp(block.timestamp + configTimelock.timelockDelay());
    ...
    function test_givenYieldAssetBpsChangesAfterRecipientQueue_reverts() public {
        ...
        vm.startPrank(admin);
        burnerLoansConfig.setYieldRepurchaseRecipient(currentRecipient);
        burnerLoansConfig.setYieldAssetRouting(address(usds), _repurchaseRouting(2_500));
        vm.stopPrank();
        ...
        vm.prank(admin);
        burnerLoansConfig.setYieldAssetRouting(address(usds), _repurchaseRouting(5_000));
        ...
        assertEq(
            burnerLoans.getYieldRepurchaseRecipient(),
            currentRecipient,
            "current recipient retained"
        );
        _assertRepurchaseRouting(address(usds), 5_000);
    }

    function test_givenDirectAllocationOrderChangesAfterRecipientQueue_reverts() public {
        IBurnerLoans.AssetYieldRouting memory routing = _directRouting(2);
        vm.prank(admin);
        ...
        IBurnerLoans.DirectYieldAllocation memory first = routing.directAllocations[0];
        routing.directAllocations[0] = routing.directAllocations[1];
        routing.directAllocations[1] = first;
        vm.prank(admin);
        burnerLoansConfig.setYieldAssetRouting(address(usds), routing);
        bytes32 currentHash = _yieldRoutingStateHash();
        vm.warp(block.timestamp + configTimelock.timelockDelay());
    ...
    function test_givenDifferentAssetBpsChangesAfterQueue_appliesTargetBps() public {
        ...
        vm.prank(admin);
        burnerLoansConfig.setYieldAssetRouting(address(secondAsset), _repurchaseRouting(2_500));
        vm.warp(block.timestamp + configTimelock.timelockDelay());

        configTimelock.executeQueuedAction(actionId);

        _assertRepurchaseRouting(address(usds), 5_000);
        _assertRepurchaseRouting(address(secondAsset), 2_500);
    }

    function test_givenReplacementRecipientRouteDisabledAfterQueue_reverts() public {
        address currentRecipient = address(_deployUsdsYieldRecipient());
        MockYieldRepurchaseRecipient replacementRecipient = _deployUsdsYieldRecipient();
        vm.startPrank(admin);
        burnerLoansConfig.setYieldRepurchaseRecipient(currentRecipient);
        burnerLoansConfig.setYieldAssetRouting(address(usds), _repurchaseRouting(5_000));
        vm.stopPrank();
        ...
        replacementRecipient.setVaultConfig(address(usdsVault), address(usds), false);
        vm.warp(block.timestamp + configTimelock.timelockDelay());
