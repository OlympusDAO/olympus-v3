...
contract YRFTimelockTests_QueueDecreaseNextYield is YRFTimelockTestBase {
    ...
    function test_givenUnauthorizedCaller_reverts(address caller_) public {
        ...
        yrfTimelock.queueDecreaseNextYield(address(sReserve), _STORED_NEXT_YIELD, 40e18);
    }
    ...
    function test_givenExpectedNextYieldMismatch_reverts(uint256 expectedNextYield_) public {
        ...
        yrfTimelock.queueDecreaseNextYield(address(sReserve), expectedNextYield_, 0);
    }
    ...
    function test_givenNewNextYieldEqualToStored_reverts() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                IYieldRepurchaseFacilityV2.IYieldRepurchaseFacilityV2_NextYieldNotDecreased.selector,
                address(sReserve),
                _STORED_NEXT_YIELD,
                _STORED_NEXT_YIELD
            )
        );
        yrfTimelock.queueDecreaseNextYield(address(sReserve), _STORED_NEXT_YIELD, _STORED_NEXT_YIELD);
    }
    ...
    function test_givenNewNextYieldAboveStored_reverts(uint256 newNextYield_) public {
        ...
        yrfTimelock.queueDecreaseNextYield(address(sReserve), _STORED_NEXT_YIELD, newNextYield_);
    }
    ...
    function test_givenTwoPendingDecreases_secondExecutionReverts() public {
        _registerBackingAsset(yieldRepo, _STORED_NEXT_YIELD);
        uint64 firstActionId = _queueDecreaseNextYield(address(sReserve), _STORED_NEXT_YIELD, 40e18);
