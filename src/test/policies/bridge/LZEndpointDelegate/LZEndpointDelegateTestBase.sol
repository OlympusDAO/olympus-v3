...
import {ADMIN_ROLE, BRIDGE_ADMIN_ROLE, BRIDGE_CHANNEL_MANAGER_ROLE, BRIDGE_CONFIGURATOR_ROLE, EMERGENCY_ROLE} from "src/policies/utils/RoleDefinitions.sol";
...
///      built Kernel + OlympusRoles + RolesAdmin stack, points at a `MockLZBridgeGateway` that
///      mirrors the real gateway's `LZ_ENDPOINT()` view, and is enabled by default so the
///      role-gated mutator tests do not need to repeat the enable call.
contract LZEndpointDelegateTestBase is Test {
    ...
    address channelManager = makeAddr("channelManager");
    ...
    function setUp() public virtual {
        ...
        rolesAdmin.grantRole(BRIDGE_CHANNEL_MANAGER_ROLE, channelManager);
    ...
    /// @dev Disables the delegate as the emergency actor. Used by tests that exercise the
    ///      `givenEnabled` revert path on the mutator surface.
    function _disableDelegate() internal {
        vm.prank(emergency);
        lzDelegate.disable("");
    }
}
