...
/// @notice Shared abstract base of the CCIPBridgeConfigTimelock test suite.
...
///         CCIPBridgeConfig over a real Chainlink lock/release pool with liquidity acceptance
...
///         CCIPBridgeConfigTest defaults verbatim: selectors 1111/2222, outbound
...
abstract contract CCIPBridgeConfigTimelockTest is Test {
    // ========== KERNEL STACK ========== //
    ...
    function _newTimelock(
        ...
        uint48 delay_,
        uint32 gracePeriod_
    ) internal returns (CCIPBridgeConfigTimelock newTimelock) {
        newTimelock = new CCIPBridgeConfigTimelock(kernel, config_, delay_, gracePeriod_);
    ...
    function _deployStackOnKernel(
        Kernel kernel_
    ) internal returns (CCIPBridgeConfig stackConfig, CCIPBridgeConfigTimelock stackTimelock) {
        stackConfig = new CCIPBridgeConfig(kernel_, address(pool), GRACE_PERIOD);
        vm.label(address(stackConfig), "secondConfig");
        stackTimelock = new CCIPBridgeConfigTimelock(
            kernel_,
            address(stackConfig),
            TIMELOCK_DELAY,
            GRACE_PERIOD
    ...
    function _deployStackOnForeignKernel()
        ...
        returns (
            ...
            CCIPBridgeConfig foreignConfig,
            CCIPBridgeConfigTimelock foreignTimelock
        )
    {
    ...
    function _addChainBatchAction(
        ...
        return _batchAction(ICCIPBridgeConfig.addChain.selector, abi.encode(update_));
    }

    function _removeChainBatchAction(
        uint64 chainSelector_
    ) internal view returns (ITimelockBatchQueue.BatchAction memory) {
        return _batchAction(ICCIPBridgeConfig.removeChain.selector, abi.encode(chainSelector_));
    }
