...
contract CCIPBridgeConfigTimelockTests_supportsInterface is CCIPBridgeConfigTimelockTest {
    // ========== FILE-LOCAL HELPERS ========== //
    ...
    function _advertisedInterfaceIds() internal pure returns (bytes4[] memory interfaceIds) {
        ...
        interfaceIds[0] = type(ICCIPBridgeConfigTimelock).interfaceId;
    ...
    //   [X] it answers true for ICCIPBridgeConfigTimelock, IVersioned,
    //       IConfigTimelockBatchQueue, ITimelockBatchQueue, IGracePeriod, IReEnabler,
    //       IEnablerV2, IEnabler and IERC165
    //   [X] it answers false for ICCIPBridgeConfig and IConfigOperator
    ...
    function test_whenInterfaceIsSupported() public view {
        assertTrue(
            timelock.supportsInterface(type(ICCIPBridgeConfigTimelock).interfaceId),
            "ICCIPBridgeConfigTimelock should be advertised"
        );
        assertTrue(
        ...
        // The constructor of another timelock demands ICCIPBridgeConfig, IConfigOperator and
        // IEnabler together; this policy advertises only the last of the three
        assertFalse(
            timelock.supportsInterface(type(ICCIPBridgeConfig).interfaceId),
            "ICCIPBridgeConfig should not be advertised"
        );
        assertFalse(
