...
// Contracts
import {CCIPBridgeConfigTimelockHandler} from "./CCIPBridgeConfigTimelockHandler.sol";
import {CCIPBridgeConfigTimelockTest} from "../CCIPBridgeConfigTimelockTest.sol";

/// @notice Stateful invariant suite for CCIPBridgeConfigTimelock. The fuzzer drives the
...
contract CCIPBridgeConfigTimelockTests_Invariants is CCIPBridgeConfigTimelockTest {
    CCIPBridgeConfigTimelockHandler internal handler;
    ...
    function setUp() public override {
        ...
        handler = new CCIPBridgeConfigTimelockHandler(
            timelock,
            config,
            ohm,
            ccipRouter,
    ...
    function invariant_registryMatchesStoredActions() public view {
        ...
        for (uint256 i; i < count; ++i) {
            CCIPBridgeConfigTimelockHandler.GhostAction memory ghost = handler.ghostActionAt(i);
    ...
    function _reconcileLiveAction(
        ...
        for (uint256 s; s < subCount_; ++s) {
            CCIPBridgeConfigTimelockHandler.GhostSub memory sub = handler.ghostSubAt(index_, s);
            ...
            for (uint256 k; k < stateCount; ++k) {
                CCIPBridgeConfigTimelockHandler.GhostState memory state = handler.ghostStateAt(
                    index_,
                    s,
                    k
                );
