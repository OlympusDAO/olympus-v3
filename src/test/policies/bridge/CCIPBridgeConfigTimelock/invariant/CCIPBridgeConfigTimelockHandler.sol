...
/// @notice Invariant-test handler that drives CCIPBridgeConfigTimelock through its production
...
contract CCIPBridgeConfigTimelockHandler is Test {
    ...
    CCIPBridgeConfigTimelock public immutable timelock;
    ...
    function queueRouteAddition(uint256 selectorSeed_, uint256 shapeSeed_) external {
        ...
        _finishRegistration(index, actionId);
        _registerSub(index, ICCIPBridgeConfig.addChain.selector, abi.encode(update));
    ...
    function queueRouteRemoval(uint256 selectorSeed_) external {
        ...
        _finishRegistration(index, actionId);
        _registerSub(index, ICCIPBridgeConfig.removeChain.selector, abi.encode(selector));
        _registerRouteStates(index, 0, selector);
        queuesSucceeded += 1;
    }
    ...
    function queueBatchPair(uint256 seed_) external {
        ...
        if (foundRl) {
            batch[cursor] = ITimelockBatchQueue.BatchAction({
                ...
                selector: ICCIPBridgeConfig.setChainRateLimits.selector,
        ...
        cursor = 0;
        if (foundRl) {
            _registerSub(index, ICCIPBridgeConfig.setChainRateLimits.selector, batch[0].payload);
            _registerState(index, cursor, 0, rlSelector);
            cursor += 1;
        }
        if (foundRp) {
            _registerSub(index, ICCIPBridgeConfig.addRemotePool.selector, batch[cursor].payload);
