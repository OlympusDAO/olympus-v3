...
contract CCIPBridgeConfigTimelockTests_queueSetRemoteToken is CCIPBridgeConfigTimelockTest {
    ...
    function test_whenTokenIsEmpty_reverts() public givenEnabled givenChainAdded {
        vm.expectRevert(
            abi.encodeWithSelector(ICCIPBridgeConfig.CCIPBridgeConfig_RemoteTokenEmpty.selector)
        );
        vm.prank(bridgeAdmin);
        timelock.queueSetRemoteToken(CHAIN_SELECTOR_A, "");
        ...
        givenEnabled
    {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(ICCIPBridgeConfig.CCIPBridgeConfig_RateLimiterDisabled.selector)
        );
        vm.prank(bridgeAdmin);
        timelock.queueSetRemoteToken(CHAIN_SELECTOR_A, remoteToken);
    }
