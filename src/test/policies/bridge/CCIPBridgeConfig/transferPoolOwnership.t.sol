...
contract CCIPBridgeConfigTests_transferPoolOwnership is CCIPBridgeConfigTest {
    ...
    // when the new owner is the zero address
    //   [X] it reverts with CCIPBridgeConfig_InvalidAddress("newOwner")
    ...
    function test_whenCallerIsAdmin() public givenEnabled givenPoolOwnershipAccepted {
        ...
        emit ICCIPBridgeConfig.PoolOwnershipTransferRequested(thirdParty);
        vm.prank(admin);
        config.transferPoolOwnership(thirdParty);
    ...
    function test_whenNewOwnerIsAnyAddress(
        ...
        vm.expectEmit(true, true, true, true, address(config));
        emit ICCIPBridgeConfig.PoolOwnershipTransferRequested(newOwner_);
        vm.prank(admin);
    ...
    function test_givenPendingTransferExists() public givenEnabled givenPoolOwnershipAccepted {
        ...
        vm.expectEmit(true, true, true, true, address(config));
        emit ICCIPBridgeConfig.PoolOwnershipTransferRequested(correctedOwner);
        vm.prank(admin);
