...
contract CCIPBridgeConfigTests_constructor is CCIPBridgeConfigTest {
    ...
    function test_whenGracePeriodIsZero_reverts() public {
        ...
        new CCIPBridgeConfig(kernel, address(pool), 0);
    }
    ...
    function test_whenGracePeriodIsZero_whenPoolIsZeroAddress_reverts() public {
        vm.expectRevert(abi.encodeWithSelector(IGracePeriod.GracePeriod_ZeroPeriod.selector));
        new CCIPBridgeConfig(kernel, address(0), 0);
    }

    // when the pool is the zero address
    //   [X] it reverts with CCIPBridgeConfig_InvalidAddress("pool")
    ...
    function test_whenPoolIsZeroAddress_reverts() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                ICCIPBridgeConfig.CCIPBridgeConfig_InvalidAddress.selector,
                "pool"
            )
        );
        new CCIPBridgeConfig(kernel, address(0), GRACE_PERIOD);
    }

    // when the pool candidate holds no code
    //   [X] it reverts with CCIPBridgeConfig_InvalidPool
    ...
    function test_whenPoolHasNoCode_reverts() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ICCIPBridgeConfig.CCIPBridgeConfig_InvalidPool.selector,
                candidate
            )
        );
        new CCIPBridgeConfig(kernel, candidate, GRACE_PERIOD);
    }
    ...
    function test_whenPoolDoesNotImplementERC165_reverts() public {
        ...
        new CCIPBridgeConfig(kernel, address(ohm), GRACE_PERIOD);
    }
    ...
    function test_whenPoolDoesNotAdvertiseCCIPPoolV1_reverts() public {
        ...
        new CCIPBridgeConfig(kernel, address(candidate), GRACE_PERIOD);
    }
    ...
    function test_whenParametersAreValid() public {
        ...
        vm.recordLogs();
        CCIPBridgeConfig freshConfig = _newConfig(address(pool), GRACE_PERIOD);
    ...
    function test_whenPoolIsBurnMintPool() public {
        CCIPBridgeConfig freshConfig = _newConfig(address(burnMintPool), GRACE_PERIOD);
    ...
    function test_whenGracePeriodIsOne() public {
        CCIPBridgeConfig freshConfig = _newConfig(address(pool), 1);
    ...
    function test_whenGracePeriodIsMax() public {
        CCIPBridgeConfig freshConfig = _newConfig(address(pool), type(uint32).max);
        assertEq(
            freshConfig.gracePeriod(),
            type(uint32).max,
    ...
    function test_whenGracePeriodIsNonZero(uint32 gracePeriod_) public {
        ...
        CCIPBridgeConfig freshConfig = _newConfig(address(pool), boundedGracePeriod);
        assertEq(
            freshConfig.gracePeriod(),
            boundedGracePeriod,
