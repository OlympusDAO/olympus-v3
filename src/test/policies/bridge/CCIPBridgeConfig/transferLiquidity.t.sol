...
contract CCIPBridgeConfigTests_transferLiquidity is CCIPBridgeConfigTest {
    ...
    //   [X] it reverts with CCIPBridgeConfig_NotLiquidityContainer
        ...
        givenEnabled
    {
        vm.expectRevert(
            abi.encodeWithSelector(
                ICCIPBridgeConfig.CCIPBridgeConfig_NotLiquidityContainer.selector
            )
        );
        vm.prank(admin);
        config.transferLiquidity(makeAddr("someSource"), TRANSFER_AMOUNT);
    ...
    // when the from address is zero
    //   [X] it reverts with CCIPBridgeConfig_InvalidAddress("from")
    ...
    // when the amount is zero
    //   [X] it reverts with CCIPBridgeConfig_ZeroAmount
        ...
        givenLiquiditySourceConfigured
    {
        ...
        emit ICCIPBridgeConfig.PoolLiquidityTransferred(address(sourcePool), TRANSFER_AMOUNT);
        vm.prank(admin);
        config.transferLiquidity(address(sourcePool), TRANSFER_AMOUNT);
    ...
    function test_whenFromIsThePoolItself() public givenEnabled givenPoolOwnershipAccepted {
        ...
        vm.expectEmit(true, true, true, true, address(config));
        emit ICCIPBridgeConfig.PoolLiquidityTransferred(address(pool), TRANSFER_AMOUNT);
        vm.prank(admin);
