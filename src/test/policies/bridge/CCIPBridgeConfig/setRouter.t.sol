...
contract CCIPBridgeConfigTests_setRouter is CCIPBridgeConfigTest {
    ...
    // when the router is the zero address
    //   [X] it reverts with CCIPBridgeConfig_InvalidAddress("router")
    ...
    //   [X] it reverts with CCIPBridgeConfig_InvalidRouter
    ...
    function test_whenRouterHasNoCode_reverts() public givenEnabled givenPoolOwnershipAccepted {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ICCIPBridgeConfig.CCIPBridgeConfig_InvalidRouter.selector,
                candidate
            )
        );
        vm.prank(admin);
    ...
    function test_whenCallerIsAdmin() public givenEnabled givenPoolOwnershipAccepted {
        ...
        emit ICCIPBridgeConfig.PoolRouterSet(candidate);
        vm.prank(admin);
        config.setRouter(candidate);
