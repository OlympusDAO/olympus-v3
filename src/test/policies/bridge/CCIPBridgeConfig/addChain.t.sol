...
contract CCIPBridgeConfigTests_addChain is CCIPBridgeConfigTest {
    ...
    // when the remote pool list is empty
    //   [X] it reverts with CCIPBridgeConfig_RemotePoolsEmpty
    //   [X] validateAddChain reverts with the same error
        ...
        givenPoolOwnershipAccepted
    {
        ...
        update.remotePoolAddresses = new bytes[](0);
        bytes memory err = abi.encodeWithSelector(
            ICCIPBridgeConfig.CCIPBridgeConfig_RemotePoolsEmpty.selector
        );
    ...
    function test_whenCallerIsAdmin() public givenEnabled givenPoolOwnershipAccepted {
        ...
        emit ICCIPBridgeConfig.RouteAdded(CHAIN_SELECTOR_A, update);
        vm.prank(admin);
        config.addChain(update);
