...
contract CCIPBridgeConfigTests_setRemoteToken is CCIPBridgeConfigTest {
    ...
    // when the remote token is empty
    //   [X] it reverts with CCIPBridgeConfig_RemoteTokenEmpty
    //   [X] validateSetRemoteToken reverts with the same error
    ...
    {
        bytes memory err = abi.encodeWithSelector(
            ICCIPBridgeConfig.CCIPBridgeConfig_RemoteTokenEmpty.selector
        );
    ...
    //   [X] it reverts with CCIPBridgeConfig_RemoteTokenUnchanged
    //   [X] validateSetRemoteToken reverts with the same error
    ...
    {
        bytes memory err = abi.encodeWithSelector(
            ICCIPBridgeConfig.CCIPBridgeConfig_RemoteTokenUnchanged.selector
        );
    ...
    {
        vm.expectRevert(
            abi.encodeWithSelector(ICCIPBridgeConfig.CCIPBridgeConfig_RemoteTokenUnchanged.selector)
        );
        vm.prank(admin);
        config.setRemoteToken(CHAIN_SELECTOR_A, REMOTE_TOKEN);
    }
    ...
    // given the outbound bucket is disabled
    //   [X] it reverts with CCIPBridgeConfig_RateLimiterDisabled
    //   [X] validateSetRemoteToken reverts with the same error
    ...
    {
        bytes memory err = abi.encodeWithSelector(
            ICCIPBridgeConfig.CCIPBridgeConfig_RateLimiterDisabled.selector
        );
        ...
        givenChainAdded
    {
        ...
        emit ICCIPBridgeConfig.RemoteTokenSet(CHAIN_SELECTOR_A, REMOTE_TOKEN, newRemoteToken);
        vm.prank(admin);
        config.setRemoteToken(CHAIN_SELECTOR_A, newRemoteToken);
        ...
        givenChainAdded
    {
        ...
        _assertLogHeader(
            logs[10],
            address(config),
            ICCIPBridgeConfig.RemoteTokenSet.selector,
            "log 10"
        );
