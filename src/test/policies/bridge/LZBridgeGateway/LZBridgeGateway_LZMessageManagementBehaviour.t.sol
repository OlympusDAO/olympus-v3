...
/// @dev Behaviour-level coverage for LZ V2 message management (skip, nilify, burn, clear)
///      executed against the real mock EndpointV2.
///
///      Each test exercises the operation through the LZEndpointDelegate policy (the gateway's
///      endpoint delegate) and asserts the resulting endpoint state: payload hash, lazy
///      inbound nonce, deliverability of the affected packet, and recipient OHM balance.
///      The corresponding access-control checks (admin/bridge_admin/unauthorised) and
///      pure proxy semantics are covered separately by LZEndpointDelegate_MessageManagement.t.sol.
///      Realistic recovery scenarios with specific failure causes (wrong peer, supply
///      underflow, compromised DVN, etc.) live in
///      LZBridgeGateway_RecoveryAfterUndeliverableMessages.t.sol and
///      LZBridgeGateway_RetryingFailedMessages.t.sol.
contract LZBridgeGatewayTests_LZMessageManagementBehaviour is LZBridgeGatewayTestBase_LZMessaging {
    // ========== SKIP ========== //
    ...
    function test_skip_preventsVerificationAndDelivery() external {
        // 1. Send a message (nonce 1 queued, not yet verified in the endpoint)
        ...
        // 3. Attempt delivery: verifyPackets tries to verify+deliver nonce 1, but nonce 1
        //    was already consumed by skip, so verification fails.
    ...
    function test_nilify_preventsDeliveryOfVerifiedMessage() external {
        ...
        // 3. Attempt delivery: a hash mismatch (NIL != real hash) causes a revert.
    ...
    ///         Flow: verify -> nilify -> skip(nonce+1) to advance the lazy nonce -> burn.
    function test_burn_permanentlyDeletesPayloadHash() external {
        ...
        // 6. Delivery is now impossible: the hash is deleted, nothing to match.
        bool delivered = _tryDeliverPacket(packetBytes);
    ...
    function test_clear_consumesMessageWithoutExecution() external {
        ...
        // 2. Clear the message (consume without executing) via the destination's delegate policy.
        ...
        // 5. Re-delivery also fails (the hash is already deleted).
        bool delivered = _tryDeliverPacket(packetBytes);
