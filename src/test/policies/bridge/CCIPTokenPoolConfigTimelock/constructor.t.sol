...
contract CCIPTokenPoolConfigTimelockTests_constructor is CCIPTokenPoolConfigTimelockTest {
    ...
    function test_whenGracePeriodIsZero_reverts() public {
        ...
        new CCIPTokenPoolConfigTimelock(kernel, address(config), TIMELOCK_DELAY, 0);
    }
    ...
    function test_whenGracePeriodIsZero_whenDelayIsBelowMinimum_reverts() public {
        vm.expectRevert(abi.encodeWithSelector(IGracePeriod.GracePeriod_ZeroPeriod.selector));
        new CCIPTokenPoolConfigTimelock(kernel, address(config), 1 days - 1, 0);
    }
    ...
    function test_whenDelayIsBelowMinimum_reverts() public {
        ...
        new CCIPTokenPoolConfigTimelock(kernel, address(config), 1 days - 1, GRACE_PERIOD);
    }
    ...
    function test_whenDelayIsAboveMaximum_reverts() public {
        ...
        new CCIPTokenPoolConfigTimelock(kernel, address(config), 30 days + 1, GRACE_PERIOD);
    }
    ...
    function test_whenDelayIsUint48Max_reverts() public {
        ...
        new CCIPTokenPoolConfigTimelock(kernel, address(config), type(uint48).max, GRACE_PERIOD);
    }
    ...
    function test_whenDelayIsBelowMinimum_whenConfigIsZeroAddress_reverts() public {
        _expectRevertTimelockDelayInvalid(1 days - 1);
        new CCIPTokenPoolConfigTimelock(kernel, address(0), 1 days - 1, GRACE_PERIOD);
    }

    // when the config is the zero address
    //   [X] it reverts with CCIPTokenPoolConfigTimelock_InvalidAddress("config")
    ...
    function test_whenConfigIsZeroAddress_reverts() public {
        vm.expectRevert(
            abi.encodeWithSelector(
                ICCIPTokenPoolConfigTimelock.CCIPTokenPoolConfigTimelock_InvalidAddress.selector,
                "config"
            )
        );
        new CCIPTokenPoolConfigTimelock(kernel, address(0), TIMELOCK_DELAY, GRACE_PERIOD);
    }

    // when the config candidate holds no code
    //   [X] it reverts with CCIPTokenPoolConfigTimelock_InvalidConfig
    ...
    function test_whenConfigHasNoCode_reverts() public {
        ...
        vm.expectRevert(
            abi.encodeWithSelector(
                ICCIPTokenPoolConfigTimelock.CCIPTokenPoolConfigTimelock_InvalidConfig.selector,
                candidate
            )
        );
        new CCIPTokenPoolConfigTimelock(kernel, candidate, TIMELOCK_DELAY, GRACE_PERIOD);
    }
    ...
    function test_whenConfigDoesNotImplementERC165_reverts() public {
        ...
        new CCIPTokenPoolConfigTimelock(kernel, address(ohm), TIMELOCK_DELAY, GRACE_PERIOD);
    }

    // when the config candidate answers ERC165 but does not advertise ICCIPTokenPoolConfig
    //   [X] it reverts with CCIPTokenPoolConfigTimelock_InvalidConfig
    ...
    function test_whenConfigDoesNotAdvertiseBridgeConfigInterface_reverts() public {
        ...
        new CCIPTokenPoolConfigTimelock(kernel, address(candidate), TIMELOCK_DELAY, GRACE_PERIOD);
    }

    // when the config candidate answers ERC165 but does not advertise IConfigOperator
    //   [X] it reverts with CCIPTokenPoolConfigTimelock_InvalidConfig
    // Requires the MockInterfaceSet mock advertising only ICCIPTokenPoolConfig and IEnabler
    function test_whenConfigDoesNotAdvertiseConfigOperatorInterface_reverts() public {
        bytes4[] memory advertised = new bytes4[](2);
        advertised[0] = type(ICCIPTokenPoolConfig).interfaceId;
        advertised[1] = type(IEnabler).interfaceId;
        MockInterfaceSet candidate = new MockInterfaceSet(advertised);
    ...
    //   [X] it reverts with CCIPTokenPoolConfigTimelock_InvalidConfig
    // Requires the MockInterfaceSet mock advertising only ICCIPTokenPoolConfig and IConfigOperator
    ...
    //   [X] it reverts with CCIPTokenPoolConfigTimelock_KernelMismatch carrying the foreign kernel
    ...
    function test_givenConfigReportsDifferentKernel_reverts() public {
        (Kernel foreignKernel, CCIPTokenPoolConfig foreignConfig, ) = _deployStackOnForeignKernel();

        vm.expectRevert(
            abi.encodeWithSelector(
                ICCIPTokenPoolConfigTimelock.CCIPTokenPoolConfigTimelock_KernelMismatch.selector,
                address(foreignKernel)
            )
        );
        new CCIPTokenPoolConfigTimelock(
            kernel,
            address(foreignConfig),
            TIMELOCK_DELAY,
            GRACE_PERIOD
        );
    }
    ...
    function test_whenParametersAreValid() public {
        vm.recordLogs();
        CCIPTokenPoolConfigTimelock freshTimelock = _newTimelock(
            address(config),
            TIMELOCK_DELAY,
            GRACE_PERIOD
        );
    ...
    function test_givenConfigIsInactiveAndDisabled() public {
        ...
        (
            CCIPTokenPoolConfig freshConfig,
            CCIPTokenPoolConfigTimelock freshTimelock
    ...
    function test_whenKernelIsZeroAddress_givenConfigKernelIsZero() public {
        CCIPTokenPoolConfig zeroKernelConfig = new CCIPTokenPoolConfig(
            Kernel(address(0)),
            address(pool),
            GRACE_PERIOD
        );
        ...
        CCIPTokenPoolConfigTimelock zeroKernelTimelock = new CCIPTokenPoolConfigTimelock(
            Kernel(address(0)),
    ...
    function test_givenTimelockAlreadyExistsForConfig() public {
        ...
        CCIPTokenPoolConfigTimelock secondTimelock = _newTimelock(
            address(config),
            TIMELOCK_DELAY,
            GRACE_PERIOD
        );
