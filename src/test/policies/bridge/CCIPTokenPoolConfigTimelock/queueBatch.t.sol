...
contract CCIPTokenPoolConfigTimelockTests_queueBatch is CCIPTokenPoolConfigTimelockTest {
    // ========== FILE-LOCAL HELPERS ========== //
    ...
    function test_whenTargetIsNotConfig_reverts() public givenEnabled givenChainAdded {
        ...
        _expectRevertActionInvalid(thirdParty, ICCIPTokenPoolConfig.setChainRateLimits.selector);
        vm.prank(bridgeAdmin);
        timelock.queueBatch(batch);
    }
    ...
    function test_whenSelectorIsUnsupported_reverts() public givenEnabled givenChainAdded {
        ITimelockBatchQueue.BatchAction[] memory batch = _singleActionBatch(
            _batchAction(ICCIPTokenPoolConfig.disableChain.selector, abi.encode(CHAIN_SELECTOR_A))
        );

        _expectRevertActionInvalid(address(config), ICCIPTokenPoolConfig.disableChain.selector);
        vm.prank(bridgeAdmin);
        timelock.queueBatch(batch);
    }
    ...
    function test_whenPayloadIsUndecodable_reverts() public givenEnabled givenChainAdded {
        ...
        ITimelockBatchQueue.BatchAction[] memory batch = _singleActionBatch(
            _batchAction(ICCIPTokenPoolConfig.removeChain.selector, hex"12")
        );
    ...
    function test_whenPayloadHasTrailingBytes_reverts() public givenEnabled givenChainAdded {
        ...
        ITimelockBatchQueue.BatchAction[] memory batch = _singleActionBatch(
            _batchAction(ICCIPTokenPoolConfig.removeChain.selector, payload)
        );

        _expectRevertActionInvalid(address(config), ICCIPTokenPoolConfig.removeChain.selector);
        vm.prank(bridgeAdmin);
        timelock.queueBatch(batch);
    }
    ...
    function test_whenPayloadUsesNonStandardOffsets_reverts() public givenEnabled givenChainAdded {
        ...
        ITimelockBatchQueue.BatchAction[] memory batch = _singleActionBatch(
            _batchAction(ICCIPTokenPoolConfig.setRemoteToken.selector, payload)
        );

        _expectRevertActionInvalid(address(config), ICCIPTokenPoolConfig.setRemoteToken.selector);
        vm.prank(bridgeAdmin);
        timelock.queueBatch(batch);
    }
