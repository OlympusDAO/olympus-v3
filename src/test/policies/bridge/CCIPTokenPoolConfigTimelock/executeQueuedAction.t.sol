...
contract CCIPTokenPoolConfigTimelockTests_executeQueuedAction is CCIPTokenPoolConfigTimelockTest {
    // ========== FILE-LOCAL HELPERS ========== //
        ...
        givenEnabled
    {
        ...
        _expectStateChangedAndExecute(actionId, 0, _allowListKey(), _expectedAllowListHash());
    ...
    function test_givenChainActionReady() public givenEnabled {
        ...
        uint256 routeAddedIndex = _indexOfLog(
            logs,
            address(config),
            ICCIPTokenPoolConfig.RouteAdded.selector,
            "config RouteAdded"
        );
    ...
    function test_givenMultiDomainBatchReady() public givenEnabled givenChainAdded {
        ...
        assertLt(
            _indexOfLog(
                logs,
                address(config),
                ICCIPTokenPoolConfig.RouteRateLimitsSet.selector,
                "RouteRateLimitsSet"
            ),
            _indexOfLog(
                logs,
                address(config),
                ICCIPTokenPoolConfig.RouteRemotePoolAdded.selector,
