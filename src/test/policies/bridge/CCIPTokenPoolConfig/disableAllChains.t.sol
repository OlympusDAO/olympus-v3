...
contract CCIPTokenPoolConfigTests_disableAllChains is CCIPTokenPoolConfigTest {
        ...
        givenSecondChainAdded
    {
        ...
        vm.expectEmit(true, true, true, true, address(config));
        emit ICCIPTokenPoolConfig.RouteDisabled(CHAIN_SELECTOR_B);
        vm.prank(emergency);
        config.disableAllChains();
        ...
        givenChainAdded
    {
        ...
        Vm.Log[] memory logs = vm.getRecordedLogs();
        assertEq(
            _countLogs(logs, address(config), ICCIPTokenPoolConfig.RouteDisabled.selector),
            1,
