...
/// @notice Invariant-test handler that drives CCIPTokenPoolConfig through its production entry
...
contract CCIPTokenPoolConfigHandler is Test {
    // ========== RIG REFERENCES ========== //
    ...
    function _rejectRouterCandidate(address candidate_) internal {
        ...
        try config.setRouter(candidate_) {
            ...
            if (
                errorSelector != ICCIPTokenPoolConfig.CCIPTokenPoolConfig_InvalidRouter.selector &&
                errorSelector != ICCIPTokenPoolConfig.CCIPTokenPoolConfig_InvalidAddress.selector
            ) {
                assembly {
                    revert(add(reason_, 0x20), mload(reason_))
                }
    ...
    function _probePayload(uint256 branch_) internal view returns (bytes memory) {
        ...
        if (branch_ == 5) return abi.encodeCall(ICCIPTokenPoolConfig.acceptPoolOwnership, ());
        if (branch_ == 6)
            return abi.encodeCall(ICCIPTokenPoolConfig.transferPoolOwnership, (outsider));
        if (branch_ == 7)
            return abi.encodeCall(ICCIPTokenPoolConfig.setRouter, (address(ccipRouter)));
        if (branch_ == 8) return abi.encodeCall(ICCIPTokenPoolConfig.setRebalancer, (outsider));
        if (branch_ == 9) return abi.encodeCall(ICCIPTokenPoolConfig.setRateLimitAdmin, (outsider));
        if (branch_ == 10) {
            return abi.encodeCall(ICCIPTokenPoolConfig.transferLiquidity, (outsider, 1));
        }
        if (branch_ == 11) {
            return
                abi.encodeCall(
                    ICCIPTokenPoolConfig.addChain,
                    (
                        ICCIPTokenPoolAdmin.ChainUpdate({
                            remoteChainSelector: selector,
        ...
        if (branch_ == 12) return abi.encodeCall(ICCIPTokenPoolConfig.removeChain, (selector));
        if (branch_ == 13) {
            return abi.encodeCall(ICCIPTokenPoolConfig.setRemoteToken, (selector, hex"01"));
        }
        if (branch_ == 14) {
            return abi.encodeCall(ICCIPTokenPoolConfig.addRemotePool, (selector, hex"01"));
        }
        if (branch_ == 15) {
            return abi.encodeCall(ICCIPTokenPoolConfig.removeRemotePool, (selector, hex"01"));
        }
        if (branch_ == 16) {
            return
                abi.encodeCall(
                    ICCIPTokenPoolConfig.applyAllowListUpdates,
        ...
        if (branch_ == 17) {
            return
                abi.encodeCall(
                    ICCIPTokenPoolConfig.setChainRateLimits,
