...
import {MockConfigPriceWithoutV2, MockConfigUnsupportedFloan, MockConfigUnsupportedPrice, MockConfigUnsupportedRoles} from "src/test/policies/BurnerLoansConfig/fixtures/MockConfigModules.sol";
...
contract BurnerLoansConfigConfigureDependenciesTest is BurnerLoansTest {
    ...
    //   then it sets modules and preserves immutable dependencies
    ...
    // configureDependencies
    // given PRICE is not installed
    //  when Config is activated
    //   then activation reverts for the missing module
    function test_givenPriceMissing_reverts() public {
        Kernel localKernel = new Kernel();
        _expectMissingModuleRevert(
            localKernel,
    ...
    // configureDependencies
    // given PRICE has an unsupported version
    //  when Config is activated
    //   then activation reverts
    function test_givenPriceVersionUnsupported_reverts() public {
        Kernel localKernel = new Kernel();
        _expectInvalidVersionRevert(
            localKernel,
            new OlympusFixedTermLoan(localKernel),
            new MockConfigUnsupportedPrice(localKernel),
            new OlympusRoles(localKernel)
        );
    }

    // configureDependencies
    // given PRICE does not implement IPRICEv2
    //  when Config is activated
    //   then activation reverts
    function test_givenPriceDoesNotImplementV2_reverts() public {
        Kernel localKernel = new Kernel();
        _expectInvalidVersionRevert(
            localKernel,
            new OlympusFixedTermLoan(localKernel),
            new MockConfigPriceWithoutV2(localKernel),
            new OlympusRoles(localKernel)
        );
    }
    ...
    function _expectMissingModuleRevert(
        ...
        BurnerLoansConfig localConfig = _deployConfigWithModules(kernel_, floan_, price_, roles_);
    ...
    function _expectInvalidVersionRevert(
        Kernel kernel_,
        Module floan_,
        Module price_,
        Module roles_
    ...
    function _deployConfigWithModules(
        ...
        MockBurnerLoansPolicy facility_ = new MockBurnerLoansPolicy(kernel_);
        ...
        localConfig = new BurnerLoansConfig(
            kernel_,
            IERC20(address(ohm)),
            depositManager,
            address(facility_)
