...
contract BurnerLoansExtendTest is BurnerLoansTest {
    ...
    // - Parameters: asset is configured, onBehalfOf is owner, loan id is fixed
    // - Expected branch: authorization check passes, then placeholder reverts
    function test_extend_givenOwnerCaller_reachesPlaceholder() public {
        vm.prank(alice);
    ...
    function test_extend_givenUnauthorizedOperator_reverts() public {
        vm.prank(operator);
    ...
    // - Parameters: asset is configured, onBehalfOf is owner, loan id is fixed
    // - Expected branch: authorization check passes, then placeholder reverts
    function test_extend_givenAuthorizedOperator_reachesPlaceholder() public {
    ...
    function test_extend_givenUnauthorizedCaller_reverts(address caller_) public {
