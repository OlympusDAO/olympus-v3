...
contract BurnerLoansRequestPermissionsTest is BurnerLoansTest {
    ...
    function test_requestPermissions_requestsLifecyclePermissions() public view {
        ...
        assertEq(permissions[0].funcSelector, IFLOANv1.addCollateral.selector, "add collateral");
        assertEq(
            permissions[1].funcSelector,
            IFLOANv1.removeCollateral.selector,
            "remove collateral"
        );
        assertEq(permissions[2].funcSelector, IFLOANv1.increaseDebt.selector, "increase debt");
        assertEq(permissions[3].funcSelector, IFLOANv1.createPosition.selector, "create position");
        assertEq(permissions[4].funcSelector, IFLOANv1.decreaseDebt.selector, "decrease debt");
        assertEq(permissions[5].funcSelector, IFLOANv1.extendMaturity.selector, "extend maturity");
        assertEq(
        ...
        assertEq(
            permissions[7].funcSelector,
            IFLOANv1.setMarketManager.selector,
            "set market manager"
        );
    }
}
