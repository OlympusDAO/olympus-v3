...
contract BurnerLoansSetAssetFeeConfigTest is BurnerLoansTest {
    ...
    //  when setAssetFeeConfig is called by admin
    //   then a single-slope fee curve is accepted
    function test_givenKinkBpsAndPostKinkSlopeBpsAreZero_setsSingleSlopeConfig() public {
        IBurnerLoans.AssetFeeConfig memory config = IBurnerLoans.AssetFeeConfig({
            baseFeeBps: 0,
            kinkBps: 0,
