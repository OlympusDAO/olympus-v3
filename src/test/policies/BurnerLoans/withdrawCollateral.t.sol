...
contract BurnerLoansWithdrawCollateralTest is BurnerLoansTest {
    ...
    // - Parameters: asset is configured, onBehalfOf is owner, recipient is owner
    // - Expected branch: authorization and recipient checks pass, then placeholder reverts
    function test_withdrawCollateral_givenOwnerCaller_reachesPlaceholder() public {
        vm.prank(alice);
    ...
    // - Parameters: asset is configured, onBehalfOf is owner, recipient is operator
    ...
    // - Parameters: asset is configured, onBehalfOf is owner, recipient is operator
    // - Expected branch: authorization and recipient checks pass, then placeholder reverts
    function test_withdrawCollateral_givenAuthorizedOperatorWithOperatorRecipient_reachesPlaceholder()
        public
    {
    ...
    // - Caller: owner (`alice`)
    // - Authorization state: owner acts on own account
    // - Parameters: asset is configured, onBehalfOf is owner, recipient is zero address
    ...
    // - Parameters: asset is configured, onBehalfOf is owner, recipient is fuzzed caller
    ...
    function test_withdrawCollateral_givenUnauthorizedCaller_reverts(address caller_) public {
        ...
        vm.prank(caller_);
        vm.expectRevert(IOperatorAuth.OperatorAuth_UnauthorizedOnBehalfOf.selector);
        burnerLoans.withdrawCollateral(address(usds), 1e6, alice, caller_);
    }
}
