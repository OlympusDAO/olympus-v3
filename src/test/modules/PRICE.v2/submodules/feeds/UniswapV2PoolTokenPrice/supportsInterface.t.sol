...
// Test
import {UniswapV2PoolTokenPriceTest} from "../UniswapV2PoolTokenPrice.t.sol";
...
contract UniswapV2PoolTokenPriceSupportsInterfaceTest is UniswapV2PoolTokenPriceTest {
    ...
    function test_supportsInterface_returnsTrueForAllInterfaces() public view {
        ...
        assertTrue(uniswapSubmodule.supportsInterface(ierc165), "Should support IERC165");

        // Check ISubmodule (which inherits IVersioned)
        bytes4 isubmodule = type(ISubmodule).interfaceId;
        assertTrue(uniswapSubmodule.supportsInterface(isubmodule), "Should support ISubmodule");

        // Check IVersioned (via ISubmodule inheritance)
        bytes4 iversioned = type(IVersioned).interfaceId;
        assertTrue(uniswapSubmodule.supportsInterface(iversioned), "Should support IVersioned");
    }

    function test_supportsInterface_unsupported_returnsFalse() public view {
        bytes4 interfaceId = 0x12345678;
        assertFalse(
            uniswapSubmodule.supportsInterface(interfaceId),
