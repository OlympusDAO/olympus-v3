...
contract UniswapV3PriceSupportsInterfaceTest is UniswapV3PriceTest {
    ...
    function test_supportsInterface_returnsTrueForAllInterfaces() public view {
        ...
        assertTrue(uniSubmodule.supportsInterface(ierc165), "Should support IERC165");

        // Check ISubmodule (which inherits IVersioned)
        bytes4 isubmodule = type(ISubmodule).interfaceId;
        assertTrue(uniSubmodule.supportsInterface(isubmodule), "Should support ISubmodule");

        // Check IVersioned (via ISubmodule inheritance)
        bytes4 iversioned = type(IVersioned).interfaceId;
        assertTrue(uniSubmodule.supportsInterface(iversioned), "Should support IVersioned");
    }

    function test_supportsInterface_unsupported_returnsFalse() public view {
        bytes4 interfaceId = 0x12345678;
        assertFalse(uniSubmodule.supportsInterface(interfaceId), "Should not support unsupported");
    }
}
