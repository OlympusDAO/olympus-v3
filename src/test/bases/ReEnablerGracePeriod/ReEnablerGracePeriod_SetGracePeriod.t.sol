...
/// @dev Tests for `ReEnablerGracePeriod.setGracePeriod`, which routes through the
///      `_authorizeSetGracePeriod` hook and the shared `_setGracePeriod` helper. A
///      successful call rejects a zero window, overwrites the `gracePeriod` slot, and emits
///      the `GracePeriodSet` event.
