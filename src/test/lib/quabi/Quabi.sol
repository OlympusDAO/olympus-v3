...
library Quabi {
        ...
        returns (bytes[] memory response)
    {
        ...
        inputs[1] = "-c";
        inputs[2] = string(
            bytes.concat("./src/test/lib/quabi/jq.sh ", bytes(query), " ", bytes(path), "")
        );
    ...
    function getPath(string memory contractName) internal returns (string memory path) {
        ...
        inputs[1] = "-c";
        inputs[2] = string(
            bytes.concat("./src/test/lib/quabi/path.sh ", bytes(contractName), ".json", "")
        );
        bytes memory res = vm.ffi(inputs);
    ...
    function getSelectors(string memory query, string memory path)
        internal
    ...
    function getFunctions(string memory contractName) public returns (bytes4[] memory) {
        string
            memory query = '\'[.ast.nodes[-1].nodes[] | if .nodeType == "FunctionDefinition" and .kind == "function" then .functionSelector else empty end ]\'';
        string memory path = getPath(contractName);

        return getSelectors(query, path);
    }

    function getFunctionsWithModifier(string memory contractName, string memory modifierName)
        public
        returns (bytes4[] memory)
    {
        string memory query = string(
            bytes.concat(
                '\'[.ast.nodes[-1].nodes[] | if .nodeType == "FunctionDefinition" and .kind == "function" and ([.modifiers[] | .modifierName.name == "',
                bytes(modifierName),
