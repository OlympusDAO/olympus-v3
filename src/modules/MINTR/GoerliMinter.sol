...
contract GoerliMinter is MINTRv1 {
    ...
    constructor(Kernel kernel_, address gdao_) Module(kernel_) {
        gdao = GDAO(gdao_);
        active = true;
    }
    ...
    /// @inheritdoc MINTRv1
    function mintGdao(address to_, uint256 amount_) external override permissioned onlyWhileActive {
        ...
        gdao.mint(to_, amount_);

        emit Mint(msg.sender, to_, amount_);
    }

    /// @inheritdoc MINTRv1
    function burnGdao(address from_, uint256 amount_)
        external
        override
        permissioned
        onlyWhileActive
