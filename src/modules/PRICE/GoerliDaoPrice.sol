...
contract GoerliDaoPrice is PRICEv1 {
    // =========  STATE ========= //
    ...
    constructor(
        ...
        gdaoEthPriceFeed = gdaoEthPriceFeed_;
        gdaoEthUpdateThreshold = gdaoEthUpdateThreshold_;
        uint8 gdaoEthDecimals = gdaoEthPriceFeed.decimals();
        ...
        uint256 exponent = decimals + reserveEthDecimals - gdaoEthDecimals;
        ...
        emit UpdateThresholdsChanged(gdaoEthUpdateThreshold_, reserveEthUpdateThreshold_);
    ...
    function getCurrentPrice() public view override returns (uint256) {
        ...
        uint256 gdaoEthPrice;
        uint256 reserveEthPrice;
        {
            (
                uint80 roundId,
                int256 gdaoEthPriceInt,
                ,
                uint256 updatedAt,
                uint80 answeredInRound
            ) = gdaoEthPriceFeed.latestRoundData();
            ...
            if (
                gdaoEthPriceInt <= 0 ||
                updatedAt < block.timestamp - uint256(gdaoEthUpdateThreshold) ||
                answeredInRound != roundId
            ) revert Price_BadFeed(address(gdaoEthPriceFeed));
            gdaoEthPrice = uint256(gdaoEthPriceInt);
        ...
        // Convert to GDAO/RESERVE price
        uint256 currentPrice = (gdaoEthPrice * _scaleFactor) / reserveEthPrice;
