    ...
    function _descriptionPreamble() private pure returns (string memory) {
        return
            string.concat(
                ...
                "- A 3-day grace window bounds the `manager` (DAO MS) fast re-enable path: after the bridge is disabled, `reEnable()` can only be called within `gracePeriod` seconds of the disable, after which restoring the bridge requires a full governance enable. This window is set to 3 days on both the LZBridgeGateway and the periphery LZCrossChainBridge, extending the shorter window described in the RFC/OIP, to give the multisig signers additional time to investigate an incident and coordinate before the fast re-enable path closes.\n",
    ...
    function _descriptionSteps() private pure returns (string memory) {
        return
            string.concat(
                ...
                "2. Grant the `manager` role to the DAO MS, authorizing it to call `reEnable()` on the LZBridgeGateway within the 3-day grace window after a disable.\n",
