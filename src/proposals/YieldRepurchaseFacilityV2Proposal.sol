    ...
    function _descriptionSummary() private pure returns (string memory) {
        return
            string.concat(
                ...
                "\n",
                "The migration preserves the v1.2 economics for sUSDS through a 100% yield buyback share and a 3% initial market discount. It applies additional max price premium configuration parameter that defines the max value the bond market will pay for a single OHM token. Initial value is set to 10% (e.g. if an oracle price is $18, max paid per OHM = $19.80). The new sUSDe asset also starts with a 100% yield buyback share. Backing launches at the current liquid backing value of 12.04 USDS per OHM, replacing the 11.33 value hardcoded in v1.2.\n",
                "\n",
