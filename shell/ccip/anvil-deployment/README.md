...
5. Negative checks: the chain's readiness report is RED and `CCIPNonEthereumSetupBatch.setup` fails naming a lane while the fee budgets read the 90k default; then the outgoing burn/mint EVM lanes are mocked to 175k and the readiness report turns GREEN.
...
11. The Solana route, added after the rollout the way an operator adds it: `routes.solana` is declared on the chain in `env.json` (the mainnet Solana entry, the template of every Solana pair, and `solana` appended to the legacy chains list), `CCIPRouteReconcileBatch.reconcileRoutes` queues `addChain(solana)` on the local config timelock (a re-run proposes nothing, and so does `executeReadyActions` before the delay), the clock is warped past the delay, `executeReadyActions` opens the route, `reconcileRoutes` proposes nothing, `CCIPBridgeBatch.reconcileTrustedRemotes` adds the SVM trusted remote with an empty re-run, the readiness report stays GREEN and prints the `[INFO]` note for the solana lane (the scripts do not gate it, so it is not mocked), and the route state is asserted against `env.json`: both buckets (enabled, capacity, rate), the SVM trusted remote (the declared receiver, set) and the gas limit. The Solana side of the pair is not rehearsed.
12. Print the final authority state.
