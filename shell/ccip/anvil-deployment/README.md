...
5. Negative checks: the chain's readiness report is RED and `CCIPNonEthereumSetupBatch.setup` fails naming a lane while the fee budgets read the 90k default; then the outgoing burn/mint EVM lanes are mocked to 175k and the readiness report turns GREEN. The solana lane is not mocked: the scripts do not gate it, and the GREEN report must carry its `[INFO]` note.
6. `CCIPNonEthereumSetupBatch.setup` (legacy LZ deactivation, activations, roles, config/timelock enablement, pool acceptance, `addChain` per route, the solana route included), with an empty re-run; the setup log must carry the solana lane note and plan the solana route, the pool must support the solana selector, and it must still be disabled and unregistered afterwards.
7. `CCIPNonEthereumSetupBatch.finalize` (`pool.enable`, `setPool`), with an empty re-run.
8. Periphery: `CCIPBridgeBatch.reconcileTrustedRemotes` (which must add the SVM trusted remote next to the EVM ones) and `CCIPBridgeBatch.enable`, each with an empty re-run. The solana route state is then asserted against `env.json`: both buckets (enabled, capacity, rate), the SVM trusted remote (the declared receiver, set) and the gas limit.
