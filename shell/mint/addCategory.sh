...
# Adds a minting category
#
# Usage:
# ./addCategory.sh --category <category name> --chain <chain name> --account <cast wallet> OR --ledger <mnemonic-index> --broadcast <false> --env <file>
#
# The chain is determined automatically from block.chainid. The --chain parameter specifies the RPC URL (from foundry.toml).
#
# Examples:
# Using cast wallet:
#   ./addCategory.sh --category test --chain sepolia --account mywallet --broadcast true
#
# Using Ledger:
#   ./addCategory.sh --category test --chain sepolia --ledger 0 --broadcast true
#
...
forge script ./src/scripts/ops/Minter.s.sol:MinterScript \
    --sig "addCategory(string)()" $category \
    --rpc-url $chain $ACCOUNT_FLAG $LEDGER_FLAGS --slow -vvv \
    --sender $ACCOUNT_ADDRESS \
    $BROADCAST_FLAG

echo ""
echo "Add category complete"
